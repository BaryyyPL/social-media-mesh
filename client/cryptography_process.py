...
API_GATEWAY_PUBLIC_KEY_PATH = "api_gateway_public_key.pem"

def load_or_create_private_key():
    if os.path.exists(CLIENT_PRIVATE_KEY_PATH):
...
def load_api_gateway_public_key():
    with open(API_GATEWAY_PUBLIC_KEY_PATH, "rb") as f:
...
def handshake_receiver(public_key, private_key, api_gateway_public_key, api_gateway_socket):
    received_message = json.loads(receive_message(api_gateway_socket).decode())
    ...
    if received_message['request_code'] == '101':
        ...
        encrypted_verification_code = public_key_encrypt(api_gateway_public_key, verification_code)
        message = {
            'request': 'hand_shake',
            'request_code': '101',
            'response_code': '999',
        ...
        send_message(api_gateway_socket, json.dumps(message))
        received_message = json.loads(receive_message(api_gateway_socket).decode())
        if (received_message['request_code'] == '101'
            ...
            if verify_signature(api_gateway_public_key, signed_symmetrical_key, symmetrical_key):
                message = {
                    'request': 'hand_shake',
                    'request_code': '101',
                    'response_code': '999',
    ...
    send_message(api_gateway_socket, json.dumps(message))
    close_socket(api_gateway_socket)
    return None
